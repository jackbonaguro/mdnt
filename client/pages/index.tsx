...
const Home: React.FunctionComponent<{ name: string; email: string }> = ({
  name,
  email,
}) => {
  ...
  const receiveSettingsCollection = receiveSettings
    ? receiveSettings
        .map((i) => {
          return {
            currency: i.currency,
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() => {
          return new Notification("Donation Received", {
            body: "Click Me",
      ...
      <HF>
        ...
        {email ? (
          <VF>
            ...
            <List>{receiveSettingsCollection}</List>
            <VF>
              ...
              >
                ...
                <option value={"xpub"}>
                  Extended Public Key (Experimental)
                </option>
              </select>
              Value
              <input
            ...
            <a
              href={`https://midnight.cash/${username}`}
              style={style.link}
            >{`https://midnight.cash/${username}`}</a>
          </VF>
        ) : (
          <></>
        )}
        <VF>
          <DonationHistory email={email}></DonationHistory>
        </VF>
      </HF>
    </VF>
  );
